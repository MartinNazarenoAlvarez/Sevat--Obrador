...
-- Volcando datos para la tabla bd_obrador.empleados: ~13 rows (aproximadamente)
...
INSERT INTO `herramientas` (`idHerramienta`, `numero_serie`, `nombre`, `descripcion`, `marca`, `modelo`, `estado`, `disponible`, `activo`) VALUES
	...
	(6, 'A0102', '1', '', '1', '1', 'Algo desgastado', 1, 0),
...
CREATE PROCEDURE `psa_listarHerramientaXSerial`(
	...
	IN `_herramientaInactivasVisibles` TINYINT
)
BEGIN
	IF _herramientaInactivasVisibles = 0 THEN
        SELECT *
        FROM herramientas
        WHERE numero_serie = _serial;
    ELSE
        SELECT *
        FROM herramientas

...
-- Volcando datos para la tabla bd_obrador.asignaciones: ~3 rows (aproximadamente)
INSERT INTO `asignaciones` (`idAsignacion`, `id_herramienta`, `id_empleado`, `fechaAsignacion`, `fechaDevolucion`, `activo`) VALUES
	...
	(32, 58, 79, '2024-11-29', '2024-11-30', 1);
...
-- Volcando estructura para procedimiento bd_obrador.asp_eliminarPrestamo
DELIMITER //
CREATE PROCEDURE `asp_eliminarPrestamo`(
	IN `_prestamoID` INT
)
BEGIN
   DELETE FROM prestamos WHERE idPrestamo = _prestamoID;
END//
DELIMITER ;
...
-- Volcando datos para la tabla bd_obrador.empleados: ~38 rows (aproximadamente)
INSERT INTO `empleados` (`IdEmpleado`, `dni`, `apellido`, `nombre`, `cargo`, `telefono`, `activo`) VALUES
	...
	(80, '14237654', 'Villalba', 'Lucas', 'Administrador', '3815432178', 0);
...
INSERT INTO `herramientas` (`idHerramienta`, `numero_serie`, `nombre`, `descripcion`, `marca`, `modelo`, `estado`, `disponible`, `activo`, `prestamo_activo`) VALUES
	...
	(58, 'G1238', 'Multimetro', 'Display con rayones', 'Stanley', 'MultiPro', 'Algo desgastado', 0, 1, 0);
...
-- Volcando datos para la tabla bd_obrador.prestamos: ~2 rows (aproximadamente)
...
-- Volcando estructura para procedimiento bd_obrador.psa_obtenerPrestamosEntreFechas
DELIMITER //
CREATE PROCEDURE `psa_obtenerPrestamosEntreFechas`(
